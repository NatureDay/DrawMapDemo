...
import static com.example.administrator.myapplication.MathUtil.EARTH_RADIUS;
import static com.example.administrator.myapplication.MathUtil.clamp;
import static com.example.administrator.myapplication.MathUtil.hav;
import static com.example.administrator.myapplication.MathUtil.havDistance;
import static com.example.administrator.myapplication.MathUtil.havFromSin;
import static com.example.administrator.myapplication.MathUtil.inverseMercator;
import static com.example.administrator.myapplication.MathUtil.mercator;
import static com.example.administrator.myapplication.MathUtil.sinFromHav;
import static com.example.administrator.myapplication.MathUtil.sinSumFromHav;
import static com.example.administrator.myapplication.MathUtil.wrap;
import static com.example.administrator.myapplication.SphericalUtil.computeDistanceBetween;
...
public class PolyUtil {
    ...
    /**
     * Same as {@link #isLocationOnEdge(GeoPoint, List, boolean, double)}
    ...
    /**
     * Same as {@link #isLocationOnPath(GeoPoint, List, boolean, double)}
     *
     * with a default tolerance of 0.1 meters.
     */
    public static boolean isLocationOnPath(GeoPoint point, List<GeoPoint> polyline,
     ...
     */
    public static int locationIndexOnPath(GeoPoint point, List<GeoPoint> poly,
                                           boolean geodesic, double tolerance) {
        return locationIndexOnEdgeOrPath(point, poly, false, geodesic, tolerance);
    }

    /**
     * Same as {@link #locationIndexOnPath(GeoPoint, List, boolean, double)}
     *
     * with a default tolerance of 0.1 meters.
     */
    public static int locationIndexOnPath(GeoPoint point, List<GeoPoint> polyline,
                                           boolean geodesic) {
     ...
     *          poly.size()-2 if between poly[poly.size() - 2] and poly[poly.size() - 1]
     */
    private static int locationIndexOnEdgeOrPath(GeoPoint point, List<GeoPoint> poly, boolean closed,
                                                 boolean geodesic, double toleranceEarth) {
        int size = poly.size();
        if (size == 0) {
            return -1;
     ...
     */
    public static List<GeoPoint> simplify(List<GeoPoint> poly, double tolerance) {
        ...
        if (closedPolygon) {
            ...
            poly.add(new GeoPoint(lastPoint.getLatitude() + OFFSET, lastPoint.getLongitude() + OFFSET));
        }
        ...
        idx = 0;
        ArrayList<GeoPoint> simplifiedLine = new ArrayList<>();
        for (GeoPoint l : poly) {
            if (dists[idx] != 0) {
     ...
     * points are the same), and false if it is not
     */
    public static boolean isClosedPolygon(List<GeoPoint> poly) {
        GeoPoint firstPoint = poly.get(0);
        GeoPoint lastPoint = poly.get(poly.size()-1);
    ...
    public static double distanceToLine(final GeoPoint p, final GeoPoint start, final GeoPoint end) {
        ...
        GeoPoint sa = new GeoPoint(p.getLatitude() - start.getLatitude(), p.getLongitude() - start.getLongitude());
